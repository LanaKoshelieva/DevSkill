...
let risposta:string = prompt ("Vuoi effettuare una misurazione più precisa? (inserisci si o no)");
 ...
 function calcolaBMI (peso:number, altezza:number):number {
    let pesoKg:number = peso / 2.20462;
    let altezzaM:number = altezza / 3.28084;
    altezzaM = altezzaM / 100;
    let BMI:number = pesoKg/(altezzaM * altezzaM);
    return BMI;
}

function stampaBMI (BMI:number):void {
    if (BMI < 16.5) {
        console.log("Sottopeso severo");
    } else if (BMI >= 16.5 && BMI <= 18.4) {
        console.log("Sottopeso");
...
let BMIcalcolato = calcolaBMI(pesoLi,altezzaP);
stampaBMI(BMIcalcolato);


function calcoloCostituzione():void {
